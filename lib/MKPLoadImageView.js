...
export default class MKPLoadImageView extends Component {
    ...
    static  fullFilePath(fileName){
        return MKPLoadImageView.fileDirectory()+'/'+fileName;
    }
    
    getImageContent(source,fileName){
        
        let fullPath = MKPLoadImageView.fullFilePath(fileName);
        ...
        RNFS.stat(fullPath).then((result)=>{
                                 ...
                                 this.setState({cacheable: true, imageSource: fullPath});
                                          ...
                                          RNFS.mkdir(MKPLoadImageView.fileDirectory(), {NSURLIsExcludedFromBackupKey: true}).then(() => {
                                                                                                                                  let downloadOptions = {
                                                                                                                                  fromUrl: source.uri,
                                                                                                                                  toFile: fullPath,
                                                                                                                                  background: true,
    ...
    analysisSource(source){
        if (source !== null
            ...
            let fileName = MKPLoadImageView.fileName(source.uri);
    ...
    componentWillMount() {
        this.analysisSource(this.state.imageSource)
    }
